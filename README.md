...
2. Cloudflare Dashboard → **Workers & Pages** → **Create application** → **Pages** → 连接 GitHub 仓库。
3. Framework preset 选 **Next.js (Static HTML Export)**。
4. Build command：`npm run build`；Build output directory：`out`。
5. 在 Pages 的 Settings → Environment variables 配置：
   - `NEXT_PUBLIC_SUPABASE_URL`
   - `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`
6. 生产分支设置为 `main`。之后每次 push 到 `main` 会自动部署；分支 push 会生成 Preview Deployment。
...
5. 把 URL 与 publishable key 放入 Cloudflare 的生产和预览环境变量。绝不把 service role 放入前端或 Cloudflare Pages 公共变量。
